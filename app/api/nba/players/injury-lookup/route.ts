...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!parsed.success) {
      console.error(
        "[Injury Lookup] Validation error:",
    ...
    const { data, error } = await supabase
      .from("nba_players_hr")
      .select(
        "odds_player_id, name, injury_status, injury_notes, injury_updated_at, injury_return_date, injury_source, injury_raw_status",
      )
    ...
    (data || []).forEach((row: any) => {
      injuryMap.set(row.odds_player_id, {
        ...
        injuryUpdatedAt: row.injury_updated_at,
        injuryReturnDate: row.injury_return_date,
        injurySource: row.injury_source,
        injuryRawStatus: row.injury_raw_status,
      });
    });
