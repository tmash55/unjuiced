...
export async function GET(req: NextRequest) {
  try {
    ...
    let periodQuery = supabase
      .from("wnba_player_period_box_scores")
      .select(
    ...
    if (season) {
      periodQuery = periodQuery.eq("season", season);
    }
    ...
    if (periodError) {
      console.error("[WNBA Player Period Box Scores] period query error:", periodError.message);
      return NextResponse.json(
    ...
    if (rows.length === 0) {
      return NextResponse.json(
        { period, season: season ?? "", games: [] } satisfies PlayerPeriodBoxScoresResponse,
    ...
    const [{ data: gameRows, error: gameError }, { data: teamRows, error: teamError }] =
      await Promise.all([
        supabase
          ...
          .in("game_id", gameIds),
        supabase.from("wnba_teams").select("team_id, name, abbreviation").in("team_id", teamIds),
      ]);

    if (gameError) {
      console.error("[WNBA Player Period Box Scores] game context query error:", gameError.message);
    }

    if (teamError) {
      console.error("[WNBA Player Period Box Scores] team context query error:", teamError.message);
    }
    ...
    const games: BoxScoreGame[] = rows.map((row) => {
      ...
      return {
        ...
        date: gameDate,
        seasonType: normalizeBasketballSeasonType(row.season_type ?? game?.season_type ?? "regular", "wnba", gameDate),
    ...
    return NextResponse.json(
      { period, season: season ?? rows[0]?.season ?? "", games } satisfies PlayerPeriodBoxScoresResponse,
      {
        headers: {
    ...
    console.error("[/api/wnba/player-period-box-scores] Error:", error);
    return NextResponse.json(
