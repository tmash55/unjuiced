...
/**
 * Hit Rate Matrix API - WNBA
 * 
 * Returns hit rates across WNBA-calibrated fixed thresholds for all players on a
 * given date. Uses 2025 game logs until enough 2026 games exist, matching the
 * WNBA hit-rate table fallback.
 * 
 * Uses the same RPC function as hit-rates v2 API for consistency:
 * - get_wnba_hit_rate_profiles_fast_v3: Denormalized profiles with sel_key, player info, etc.
...
const MARKET_THRESHOLDS: Record<string, number[]> = {
  // Points: WNBA scoring tails are tighter than NBA, so stop before cells go mostly empty.
  player_points: [5, 10, 15, 20, 25, 30, 35, 40],
  
  player_rebounds: [2, 4, 6, 8, 10, 12, 14],
  
  player_assists: [2, 4, 6, 8, 10, 12],
  
  player_threes_made: [1, 2, 3, 4, 5, 6],
  
  player_blocks: [1, 2, 3, 4],
  
  player_steals: [1, 2, 3, 4],
  
  // Turnovers: 1-7 by 1s (low volume stat)
  player_turnovers: [1, 2, 3, 4, 5, 6, 7],
  
  player_points_rebounds_assists: [10, 15, 20, 25, 30, 35, 40, 45, 50],
  
  player_points_rebounds: [5, 10, 15, 20, 25, 30, 35, 40],
  
  player_points_assists: [5, 10, 15, 20, 25, 30, 35, 40],
  
  // R+A: 3-21 by 3s
  player_rebounds_assists: [3, 6, 9, 12, 15, 18, 21],
  
  player_blocks_steals: [1, 2, 3, 4, 5, 6],
};

// Fallback thresholds
const DEFAULT_THRESHOLDS = [5, 10, 15, 20, 25, 30, 35, 40];
...
function parseOddsEntry(entry: RedisOddEntry | string): RedisOddEntry | null {
  if (!entry) return null;
  if (typeof entry === "string") {
    try {
      const parsed = JSON.parse(entry);
      return parsed && typeof parsed === "object" ? parsed as RedisOddEntry : null;
    } catch {
      return null;
    }
  }
...
function getStatFromLog(log: any, market: string): number | null {
  // WNBA hit-rate profiles store market-specific logs as { stat, hit, ... }.
  if (typeof log.stat === "number" && Number.isFinite(log.stat)) {
    return log.stat;
  }
...
function getDvpQuality(dvpRank: number | null): "favorable" | "neutral" | "unfavorable" | null {
  ...
  if (dvpRank <= 4) return "unfavorable";
  if (dvpRank >= 10) return "favorable";
  return "neutral";
}

async function fetchWnbaDvpRankMap(
  supabase: ReturnType<typeof createServerSupabaseClient>,
  profiles: any[],
  market: string
): Promise<Map<string, number | null>> {
  const result = new Map<string, number | null>();
  const fields = getWnbaDvpMarketFields(market);
  if (!fields) return result;

  const opponentTeamIds = [
    ...new Set(
      profiles
        .map((profile) => profile.opponent_team_id ?? getWnbaTeamIdFromAbbr(profile.opponent_team_abbr))
        .filter(Boolean)
    ),
  ] as number[];

  if (opponentTeamIds.length === 0) return result;
  ...
  if (error) {
    console.error("[hit-rate-matrix-wnba] DvP fetch error:", error.message);
    return result;
  }
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { data: profiles, error: profilesError } = await supabase.rpc("get_wnba_hit_rate_profiles_fast_v3", {
      p_dates: [targetDate],
      p_market: market,
      p_has_odds: null, // We want all profiles, with or without live odds
      p_limit: 500,
      p_offset: 0,
    });

    if (profilesError) {
      console.error("[hit-rate-matrix-wnba] RPC error:", profilesError);
      return NextResponse.json(
    ...
    console.log(`[hit-rate-matrix-wnba] RPC returned ${profiles?.length || 0} profiles for ${targetDate}/${market}`);
    ...
    if (gameLogsError) {
      console.error("[hit-rate-matrix-wnba] Game logs fetch error:", gameLogsError.message);
    }

    const dvpRankMap = await fetchWnbaDvpRankMap(supabase, profiles, market);
    ...
    console.log(`[hit-rate-matrix-wnba] Fetched game_logs for ${gameLogsMap.size} players`);
    ...
    for (const profile of profiles) {
      ...
      const position = normalizeWnbaPosition(profile.player_depth_chart_pos || profile.player_position);
    ...
    console.log(`[hit-rate-matrix-wnba] ${oddsLookups.length} players passed filters`);
    ...
    const linesPromises = oddsLookups.map(async (lookup) => {
      const linesKey = `linesidx:wnba:${lookup.eventId}:${lookup.market}:${lookup.playerUuid}`;
    ...
    if (linesResults.length > 0) {
      ...
      console.log(`[hit-rate-matrix-wnba] Sample player ${sample.playerUuid} available lines:`, sample.availableLines.slice(0, 10));
      console.log(`[hit-rate-matrix-wnba] Thresholds for ${market}:`, thresholdsForMarket);
    }
    ...
    for (const result of linesResults) {
      for (const threshold of thresholdsForMarket) {
        ...
        if (closestLine !== null) {
          const booksKey = `booksidx:wnba:${result.eventId}:${result.market}:${result.playerUuid}:${closestLine}`;
    ...
    const oddsPromises = Array.from(uniqueEventMarkets).flatMap(em => {
      ...
      return Array.from(allBooks).map(async (book) => {
        const oddsKey = `odds:wnba:${eventId}:${mkt}:${book}`;
    ...
    console.log(`[hit-rate-matrix-wnba] Found ${allBooks.size} unique books, ${oddsMap.size} odds blobs, ${thresholdToLineMap.size} line mappings`);
    ...
    for (const result of linesResults) {
      ...
      for (const threshold of thresholdsForMarket) {
        ...
        for (const book of booksForLine) {
          ...
          for (const [, rawEntry] of Object.entries(oddsBlob)) {
            const entry = parseOddsEntry(rawEntry);
            if (!entry?.player_id) continue;
            
            // Match player and actual line (the sportsbook's line, not our threshold)
            if (entry.player_id === result.playerUuid && Number(entry.line) === actualLine && entry.side === 'over') {
      ...
      rows.push({
        ...
        nbaPlayerId: profile.nba_player_id ?? null,
    ...
    console.log(`[hit-rate-matrix-wnba] ${targetDate}/${market}: ${rows.length} players in ${responseTime}ms`);
    ...
    console.error("[hit-rate-matrix-wnba] Error:", error);
    return NextResponse.json(
