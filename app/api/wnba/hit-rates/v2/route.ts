...
async function fetchBestOddsForRows(
  ...
  const keys = validRows.map(
    (r) => `bestodds:wnba:${r.event_id}:${r.market}:${r.sel_key}`,
  );
...
async function fetchPlayerMetadata(
  ...
  const { data, error } = await supabase
    ...
    .select(
      "wnba_player_id, jersey_number, position, depth_chart_pos, nba_player_id, injury_updated_at, injury_return_date, injury_source, injury_raw_status",
    )
    .in("wnba_player_id", playerIds);

  if (error) {
    console.error(
      "[Hit Rates v2 WNBA] Player metadata fetch error:",
      error.message,
    );
    return result;
  }
  ...
  for (const player of data || []) {
    result.set(Number(player.wnba_player_id), {
      ...
      injury_updated_at: player.injury_updated_at ?? null,
      injury_return_date: player.injury_return_date ?? null,
      injury_source: player.injury_source ?? null,
      injury_raw_status: player.injury_raw_status ?? null,
    });
  }
...
function normalizeWnbaPosition(
  position?: string | null,
): "G" | "F" | "C" | null {
  if (!position) return null;
  const upper = position.toUpperCase();
  if (upper === "C") return "C";
  if (upper === "F" || upper === "SF" || upper === "PF" || upper === "FC")
    return "F";
  if (upper === "G" || upper === "PG" || upper === "SG" || upper === "GF")
    return "G";
  return null;
}

function getWnbaDvpMarketFields(
  market?: string | null,
): { rank: string; avg: string } | null {
  switch (market) {
    case "player_points":
...
async function fetchHistoricalContextForRows(
  ...
  if (profileError) {
    console.error(
      "[Hit Rates v2 WNBA] Historical profile context fetch error:",
  ...
  const playerIds = [
    ...new Set(h2hRows.map((row) => row.player_id).filter(Boolean)),
  ] as number[];
  const opponentIds = [
    ...new Set(
      h2hRows
  ...
  const { data: boxScores, error: boxScoreError } = await supabase
    ...
    .select(
      "player_id, opponent_team_id, game_date, pts, reb, ast, fg3m, stl, blk, tov",
    )
  ...
  if (boxScoreError) {
    console.error(
      "[Hit Rates v2 WNBA] H2H box score fetch error:",
  ...
  for (const row of h2hRows) {
    ...
    const games = (
      boxScoresByPlayerOpponent.get(`${row.player_id}:${opponentTeamId}`) ?? []
...
function isRegularSeasonGame(game: any): boolean {
  return String(game.season_type || "")
    .toLowerCase()
    .includes("regular");
}
...
async function getDefaultDatesToFetch(
  ...
  if (error) {
    console.error(
      "[Hit Rates v2 WNBA] Schedule date lookup error:",
      error.message,
    );
  ...
  const dates = [
    ...new Set(preferredGames.map((game) => game.game_date)),
  ].slice(0, 2);
...
function transformProfile(
  ...
  return {
    ...
    injury_updated_at:
      row.injury_updated_at ?? playerMetadata?.injury_updated_at ?? null,
    injury_return_date:
      row.injury_return_date ?? playerMetadata?.injury_return_date ?? null,
    injury_source: row.injury_source ?? playerMetadata?.injury_source ?? null,
    injury_raw_status:
      row.injury_raw_status ?? playerMetadata?.injury_raw_status ?? null,
    ...
    nba_players_hr: row.player_name
      ? {
          ...
          depth_chart_pos:
            row.player_depth_chart_pos || playerMetadata?.depth_chart_pos,
          jersey_number:
            row.jersey_number ?? playerMetadata?.jersey_number ?? null,
        }
      : null,
    nba_games_hr: {
      game_date: row.game_date,
    ...
    matchup: effectiveDvpRank
      ? {
          ...
          rank_label:
            row.dvp_label ??
            (matchupQuality === "favorable"
              ? "Weak"
              : matchupQuality === "unfavorable"
                ? "Tough"
                : "Neutral"),
...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (canUseCache) {
      ...
      try {
        ...
        if (cached && cached.data) {
          ...
          cacheHit = true;
          console.log(
            `[Hit Rates v2 WNBA] Cache HIT (${allData.length} profiles) in ${Date.now() - startTime}ms`,
          );
        }
      } catch (e) {
    ...
    if (!cacheHit) {
      ...
      console.log(
        `[Hit Rates v2 WNBA] RPC fetch: ${allData.length} profiles (${withOddsCount} with odds) in ${dbTime}ms`,
      );
    ...
    filteredData = filteredData.map((row) => {
      ...
      return {
        ...
        h2h_games:
          row.h2h_games && row.h2h_games > 0 ? row.h2h_games : context.h2hGames,
      };
    });

    const playerMetadataMap = await fetchPlayerMetadata(supabase, filteredData);
    const dvpRankMap = await fetchDvpRanksForRows(
      supabase,
      filteredData,
      playerMetadataMap,
    );
    ...
    filteredData = filteredData.map((row) => {
      ...
      const dvp =
        opponentTeamId && position && row.market
          ? (dvpRankMap.get(`${opponentTeamId}:${position}:${row.market}`) ??
            null)
          : null;
    ...
    console.log(
      `[Hit Rates v2 WNBA] Best odds fetch: ${bestOddsMap.size} keys in ${bestOddsTime}ms`,
    );
    ...
    const transformedData = paginatedData.map((row, index) => {
      ...
      const dvpRank =
        opponentTeamId && position && row.market
          ...
          : null;
      const paceContext =
        paceContextMap.get(getPaceContextKey(paceRows[index])) ?? null;
