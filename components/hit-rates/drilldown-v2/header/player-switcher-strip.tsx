...
export const PlayerSwitcherStrip = React.memo(function PlayerSwitcherStrip({
  ...
  const visiblePlayers = useMemo(() => {
    const normalizedGameId = selectedGameId ? normalizeGameId(selectedGameId) : null;
    const normalizedQuery = query.trim().toLowerCase();
    const seen = new Set<number>();

    return players
      .filter((player) => {
        if (normalizedGameId && normalizeGameId(player.gameId) !== normalizedGameId) return false;
        if (normalizedQuery && !player.playerName.toLowerCase().includes(normalizedQuery)) return false;
        if (seen.has(player.playerId)) return false;
        seen.add(player.playerId);
        return true;
      })
      .sort((a, b) => {
        const activeDelta = Number(b.playerId === activePlayerId) - Number(a.playerId === activePlayerId);
    ...
    <div className="flex items-center gap-2 py-2">
      <Tooltip content="Back to Hit Rates" side="bottom">
        <Link
          ...
          className="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-md border border-neutral-200/80 bg-white text-neutral-500 transition-colors hover:border-brand/40 hover:text-brand dark:border-neutral-800/80 dark:bg-neutral-900 dark:text-neutral-400 dark:hover:text-brand"
        >
          <ArrowLeft className="h-4 w-4" />
        </Link>
      </Tooltip>

      <label className="hidden sm:flex h-8 w-[210px] shrink-0 items-center gap-2 rounded-md border border-neutral-200/80 bg-white px-2.5 text-[12px] text-neutral-500 transition-colors focus-within:border-brand/45 focus-within:ring-2 focus-within:ring-brand/10 dark:border-neutral-800/80 dark:bg-neutral-900 dark:text-neutral-500">
        ...
        <div className="flex min-w-0 flex-1 items-center gap-1.5 overflow-x-auto scrollbar-hide">
          {isLoading ? (
            Array.from({ length: 6 }).map((_, i) => (
              <div
                key={i}
                className="hidden h-8 shrink-0 items-center gap-1.5 rounded-full border border-dashed border-neutral-200/80 bg-neutral-50/40 px-2.5 dark:border-neutral-800/80 dark:bg-neutral-900/40 md:inline-flex"
                aria-hidden="true"
              >
            ...
            visiblePlayers.map((player) => {
              ...
              const tooltipContent = injuryDot
                ? `${injuryDot.label}${player.injuryNotes ? ` — ${player.injuryNotes}` : ""}`
                : null;
              const pill = (
                <button
                ...
                >
                  <span className="relative h-5 w-5 shrink-0">
                    ...
                    {injuryDot && (
                      <span
                        className={cn(
                          "absolute -bottom-0.5 -right-0.5 h-2 w-2 rounded-full ring-[1.5px]",
                          injuryDot.bg,
            ...
            <div className="hidden h-8 items-center rounded-full border border-dashed border-neutral-200/80 px-3 text-[11px] font-bold text-neutral-400 dark:border-neutral-800/80 dark:text-neutral-500 md:inline-flex">
...
function getInjuryDot(status: string | null): { bg: string; label: string } | null {
  const s = (status ?? "").toLowerCase();
  if (s === "out") return { bg: "bg-red-500 dark:bg-red-400", label: "Out" };
  if (s === "doubtful") return { bg: "bg-red-400 dark:bg-red-300", label: "Doubtful" };
  if (s === "questionable" || s === "gtd" || s === "game time decision") {
    return { bg: "bg-amber-500 dark:bg-amber-400", label: "Game-time decision" };
  }
  if (s === "probable") return { bg: "bg-emerald-500 dark:bg-emerald-400", label: "Probable" };
  return null;
}
