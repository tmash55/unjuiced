        ...
        <div className="mt-12 grid w-full max-w-5xl grid-cols-1 gap-8 md:grid-cols-2">
          {tiers.map((tier, tierIdx) => {
            ...
            if (process.env.NODE_ENV === 'development') {
              console.log(`[Pricing] ${tier.title}: productType=${productType}, priceId=${priceId || 'EMPTY'}`);
            }
            ...
            return (
              ...
              >
                ...
                {/* NEW badge for Hit Rates */}
                {!tier.featured && (
                  <div className="absolute -top-4 left-4 md:left-1/2 md:-translate-x-1/2">
                    <span className="rounded-full bg-gradient-to-r from-teal-500 to-emerald-500 px-4 py-1 text-sm font-medium text-white shadow-lg">
                      ✨ NEW
                    </span>
                  </div>
                )}
                    ...
                    {hasTrial ? (
                      <>
                        ...
                        <ButtonLink
                          ...
                          href={getCheckoutUrl(productType, isYearly, tier.trialDays)}
                          variant="primary"
                        >
                          Start Free — {tier.trialDays}-Day Trial
                        </ButtonLink>
