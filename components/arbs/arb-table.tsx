...
export function ArbTable({
  ...
  const sortedPairs = useMemo(() => {
    ...
    const getGame = (r: ArbRow) =>
      `${r.ev?.away?.abbr || ""} @ ${r.ev?.home?.abbr || ""}`.toLowerCase();
    ...
    return [...pairs].sort((a, b) => {
      ...
      return getGame(a.r) < getGame(b.r)
        ? -1 * dir
        : getGame(a.r) > getGame(b.r)
          ? 1 * dir
          : 0;
    });
  ...
  if (!rows?.length) {
    return <div className="text-muted-foreground text-sm">No arbs yet.</div>;
  }
  ...
  const toggleSort = (col: SortBy) => {
    if (sortBy === col) setSortDir((d) => (d === "asc" ? "desc" : "asc"));
      ...
      setSortBy(col);
      setSortDir(col === "game" ? "asc" : "desc");
    }
  };

  const logo = (id?: string) => {
    return getSportsbookLogo(id);
  };
  const bookName = (id?: string) => {
    return getSportsbookName(id);
  };
    ...
    <div className="overflow-x-auto rounded-md border">
      <table className="min-w-full table-fixed text-sm">
        ...
        <thead className="bg-muted/50">
          <tr>
            <th className="w-[112px] p-2 text-center">
              <button
                type="button"
              ...
              >
                ROI %{" "}
                <ArrowUpDown
                  className={`h-3.5 w-3.5 ${sortBy === "roi" ? "opacity-100" : "opacity-40"} ${sortBy === "roi" && sortDir === "asc" ? "rotate-180" : ""}`}
                />
              </button>
            </th>
            <th className="border-l border-gray-200 p-2 text-center dark:border-slate-700">
              <button
                type="button"
              ...
              >
                Game{" "}
                <ArrowUpDown
                  className={`h-3.5 w-3.5 ${sortBy === "game" ? "opacity-100" : "opacity-40"} ${sortBy === "game" && sortDir === "asc" ? "rotate-180" : ""}`}
                />
              </button>
            </th>
            <th className="w-[112px] border-l border-gray-200 p-2 text-center dark:border-slate-700">
              <button
                type="button"
              ...
              >
                Time{" "}
                <ArrowUpDown
                  className={`h-3.5 w-3.5 ${sortBy === "time" ? "opacity-100" : "opacity-40"} ${sortBy === "time" && sortDir === "asc" ? "rotate-180" : ""}`}
                />
              </button>
            </th>
            <th className="border-l border-gray-200 p-2 text-center dark:border-slate-700">
            ...
            <th className="w-[160px] border-l border-gray-200 p-2 text-center dark:border-slate-700">
              Bet Size
            </th>
            <th className="w-[120px] border-l border-gray-200 p-2 text-center dark:border-slate-700">
              Profit
            </th>
          </tr>
        </thead>
            ...
            return (
              <tr key={id} className={`border-t`}>
                <td className={`p-2 text-center`}>
                  <span className="inline-flex items-center justify-center rounded bg-emerald-50 px-3 py-1.5 text-base font-bold text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300">
                    +{roiPct}%
                  </span>
                </td>
                <td
                  className={`border-l border-gray-200 p-2 whitespace-nowrap dark:border-slate-700`}
                >
                ...
                <td
                  className={`border-l border-gray-200 p-2 text-center whitespace-nowrap dark:border-slate-700`}
                >
                  {(r as any).ev?.live ? (
                ...
                <td
                  className={`border-l border-gray-200 p-2 align-top whitespace-nowrap dark:border-slate-700`}
                >
                  <div
                    className={`w-full rounded-md ${isHighlighted ? "ring-2 ring-emerald-400/40" : ""}`}
                  >
                    <div className="mb-2">
                      <div className="flex flex-wrap items-center justify-center gap-2">
                        <div className="inline-flex items-center gap-2 rounded-md border border-gray-200 bg-gray-50 px-3 py-1 dark:border-slate-700 dark:bg-slate-700/40">
                          <span className="text-foreground text-xs font-semibold">
                            {(() => {
                              const label = humanizeMarket(r.mkt);
                              // For moneyline/spread-style markets, show only the market label
                              if (isMoneyline(r.mkt) || isSpread(r.mkt))
                                return label;
                              const player =
                                extractPlayer(r.o?.name) ||
                                extractPlayer(r.u?.name);
                              const short = formatPlayerShort(player);
                              return short ? `${label} - ${short}` : label;
                            })()}
                          </span>
                        </div>
                        <ArbRiskIndicator row={r} />
                      </div>
                    </div>
                    <div className="relative pr-10">
                      <div className="space-y-2 pr-1">
                        <div className="flex items-center justify-between rounded-md border border-gray-200 bg-gray-50 px-2 py-2 dark:border-slate-700 dark:bg-slate-700/40">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            {(r.o?.u || getBookFallbackUrl(r.o?.bk)) && (
                              <Tooltip
                                content={`Place bet on ${bookName(r.o?.bk)}`}
                              >
                                <button
                                  type="button"
                                  onClick={() => openLink(r.o?.bk, r.o?.u)}
                                  className="inline-flex h-7 w-7 items-center justify-center rounded-md border border-gray-300 bg-gray-100 text-gray-700 hover:bg-gray-200 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 dark:hover:bg-gray-600"
                                >
                        ...
                        <div className="flex items-center justify-between rounded-md border border-gray-200 bg-gray-50 px-2 py-2 dark:border-slate-700 dark:bg-slate-700/40">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            {(r.u?.u || getBookFallbackUrl(r.u?.bk)) && (
                              <Tooltip
                                content={`Place bet on ${bookName(r.u?.bk)}`}
                              >
                                <button
                                  type="button"
                      ...
                      <Tooltip content="Open both bets">
                        <button
                          ...
                          className="absolute top-0 right-0 bottom-0 inline-flex w-10 flex-col items-center justify-center rounded-md bg-[var(--primary)] text-white shadow-sm transition-all hover:bg-[var(--primary-strong)] hover:shadow-md focus:ring-2 focus:ring-[var(--primary)] focus:ring-offset-1 focus:outline-none"
                        >
                ...
                <td
                  className={`border-l border-gray-200 p-2 text-center align-top whitespace-nowrap dark:border-slate-700`}
                >
                  {/* spacer to align with market header pill: clone the pill invisibly for exact height */}
                  <div className="mb-2 flex items-center justify-center">
                    <div className="invisible inline-flex items-center gap-2 rounded-md border border-transparent px-3 py-1">
                      <span className="text-xs font-semibold">&nbsp;</span>
                    </div>
                  </div>
                  <div className="mx-auto -mt-px inline-block min-w-[200px] rounded-xl border border-gray-200 bg-gradient-to-br from-gray-50 to-gray-100 p-3 text-left shadow-sm dark:border-gray-700 dark:from-gray-800 dark:to-gray-900">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          <input
                            type="text"
                            value={
                              customWagers[id]?.over ??
                              Math.round(plan.overStake).toString()
                            }
                            onChange={(e) => {
                              ...
                              setCustomWagers((prev) => {
                                const newState = {
                                  ...
                                  [id]: {
                                    ...
                                    under:
                                      prev[id]?.under ??
                                      Math.round(plan.underStake).toString(),
                                  },
                                };
                                console.log(
                                  "New customWagers state:",
                                  newState,
                                );
                                return newState;
                              });
                            ...
                            className="h-7 w-20 [appearance:textfield] rounded-lg border border-gray-300 bg-white px-2 text-right text-sm font-medium focus:ring-2 focus:ring-emerald-500 focus:outline-none dark:border-gray-600 dark:bg-gray-900 [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none"
                          />
                        </div>
                      </div>
                      <div className="flex items-center justify-between">
                ...
                <td
                  className={`border-l border-gray-200 p-2 text-center align-middle whitespace-nowrap dark:border-slate-700`}
                >
