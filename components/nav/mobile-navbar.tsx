...
import { createHref } from "@/components/nav/content/shared";
import { ChevronDown, Menu, X, Link2, BarChart3, Handshake, Code } from "lucide-react";
import Link from "next/link";
import { useParams } from "next/navigation";
import { ComponentProps, ReactNode, useEffect, useState } from "react";

import { ButtonProps, buttonVariants } from "@/components/button";
import { navItems, type NavTheme } from "./navbar";
        ...
        <ul className="grid divide-y divide-neutral-200 dark:divide-white/[0.15]">
          {navItems.map(({ name, href, childItems }, idx) => (
            <MobileNavItem
              ...
              name={name}
              href={href}
              childItems={childItems as NavItemChildren | undefined}
              setOpen={setOpen}
            />
          ))}
        ...
        <AnimatedSizeContainer height>
          ...
          {expanded && (
            <div className="grid grid-cols-1 gap-4 overflow-hidden py-4">
              {childItems.map((item: NavItemChild | NavItemGroup, idx: number) =>
                "items" in item ? (
                  <div key={idx} className="grid grid-cols-1 gap-3">
                    ...
                    {item.items.map((childItem: NavItemChild, childIdx: number) => (
                      <ChildItem
                        key={childIdx}
                        item={childItem}
                        setOpen={setOpen}
...
const ChildItem = ({
  ...
  const { domain = "notalus.ai" } = useParams() as { domain?: string };
  ...
  return (
    ...
    >
      <div
        className={cn(
          "flex size-10 items-center justify-center rounded-lg border border-neutral-200 bg-gradient-to-t from-neutral-100 dark:border-white/10 dark:from-neutral-800",
          size === "small" && "size-8",
        )}
      >
        {SpecialIcon ?? (
          Icon && (
            <Icon
              className={cn(
                "size-5 text-neutral-700 grayscale dark:text-white/60",
                size === "small" && "size-4",
              )}
            />
          )
