...
export function shortenPeriodPrefix(text: string): string {
  return text
    ...
    // Shorten long combo markets
    .replace(/Points \+ Rebounds \+ Assists/gi, "PRA")
    .replace(/Pts \+ Reb \+ Ast/gi, "PRA")
    .replace(/Points Rebounds Assists/gi, "PRA")
    .replace(/Points \+ Rebounds/gi, "P+R")
    .replace(/Pts \+ Reb/gi, "P+R")
    .replace(/Points Rebounds/gi, "P+R")
    .replace(/Points \+ Assists/gi, "P+A")
    .replace(/Pts \+ Ast/gi, "P+A")
    .replace(/Points Assists/gi, "P+A")
    .replace(/Rebounds \+ Assists/gi, "R+A")
    .replace(/Reb \+ Ast/gi, "R+A")
    .replace(/Rebounds Assists/gi, "R+A")
    .replace(/Blocks \+ Steals/gi, "B+S")
    .replace(/Blk \+ Stl/gi, "B+S")
    .replace(/Steals \+ Blocks/gi, "B+S")
    .replace(/Double Double/gi, "DD")
    .replace(/Triple Double/gi, "TD")
    .replace(/Fantasy Points/gi, "FAN")
    .replace(/3-Pointers Made/gi, "3PM")
    .replace(/Three Pointers Made/gi, "3PM")
    .replace(/3-Pointers/gi, "3PM")
    .replace(/Three Pointers/gi, "3PM")
    .replace(/Threes Made/gi, "3PM")
    // Remove redundant "Player" prefix
    .replace(/Player /gi, "");
}
