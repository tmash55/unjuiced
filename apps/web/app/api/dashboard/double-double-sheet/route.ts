...
import {
  computeDoubleDoubleSheet,
  getDoubleDoubleSheetCache,
  storeDoubleDoubleSheet,
  type DoubleDoubleSheetData,
} from "@/lib/sgp/double-double-sheet";
...
const L1_CACHE = new Map<string, { data: DoubleDoubleSheetData; ts: number }>();
const L1_CACHE_TTL_MS = 30_000;
const DEFAULT_CACHE_KEY = "default";

function getL1Cache(key: string): DoubleDoubleSheetData | null {
...
function setL1Cache(key: string, data: DoubleDoubleSheetData): void {
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (!fresh && !customConfig) {
      ...
      if (l1) {
        const response: DoubleDoubleSheetResponse = {
          data: l1,
          source: "l1_cache",
          timestamp: Date.now(),
        };
      ...
      const redisCached = await getDoubleDoubleSheetCache();
      if (redisCached) {
    ...
    if (!compute && !customConfig) {
      const response: DoubleDoubleSheetResponse = {
        ...
        source: "empty",
        timestamp: Date.now(),
        message: "Double-double sheet is warming. Retry shortly or use ?compute=true.",
      };
      return NextResponse.json(response, {
        headers: {
    ...
    const computed = await computeDoubleDoubleSheet(options);
    if (!customConfig) {
      await storeDoubleDoubleSheet(computed);
    ...
    console.error("[dashboard/double-double-sheet] Error:", error);
    return NextResponse.json(
      {
        error: "Failed to fetch double-double sheet",
        details:
