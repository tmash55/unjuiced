-- WNBA player box scores RPC
--
-- Run this in Supabase SQL editor.
--
-- Mirrors public.get_player_box_scores but targets the WNBA tables:
--   wnba_players_hr, wnba_teams, wnba_player_box_scores, wnba_games_hr
--
-- Differences vs. the NBA version:
--   1. Season default is '2025' (WNBA uses single-year season strings;
--      NBA uses '2025-26').
--   2. Passing p_season = NULL skips the season filter, returning the player's
--      most recent games across seasons. Enables a true rolling L10 for
--      drilldowns when no explicit season is requested.
--   3. Uses w.wnba_player_id as the player-id column on wnba_players_hr.
--      Adjust if that column is named differently in your schema.
--
-- Note: column names on wnba_player_box_scores are assumed to mirror
-- nba_player_box_scores. If any column is absent in the WNBA table, the
-- function will error when called and the missing column must be renamed
-- or dropped from this body.
...
BEGIN
    ...
    box_scores AS (
        SELECT
            ...
            -- Tracking
            bs.passes,
            bs.potential_off_reb,
            bs.potential_def_reb,
            bs.potential_reb,
            -- Combo stats
