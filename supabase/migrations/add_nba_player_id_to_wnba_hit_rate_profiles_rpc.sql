-- Add nba_player_id to get_wnba_hit_rate_profiles_fast_v3
--
-- Run this in Supabase SQL editor.
--
-- Adds nba_player_id (cdn.nba.com headshot ID) to each row by LEFT JOINing
-- wnba_players_hr on wnba_player_id. NULL for unmatched players.
--
-- DROP first because the RETURNS TABLE shape changes (new column).
...
BEGIN
  ...
  SELECT
    h.id,
    h.player_id::BIGINT,
    p.nba_player_id::BIGINT             AS nba_player_id,
    h.player_name,
  ...
  LEFT JOIN wnba_players_hr p ON p.wnba_player_id = h.player_id
  WHERE
